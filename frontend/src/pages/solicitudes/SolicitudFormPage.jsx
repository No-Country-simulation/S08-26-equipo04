...
export const SolicitudFormPage = () => {
  ...
  const clienteMode = watch('cliente_mode');
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_224px] lg:items-start">
        <div className="space-y-5">
          <Card className="p-0 overflow-hidden">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              <CardTitle className="text-label font-medium">Cliente</CardTitle>
              <div className="flex rounded-lg bg-canvas p-0.5 text-label" role="group" aria-label="Tipo de cliente">
                <button type="button" onClick={() => setValue('cliente_mode', 'registrado', { shouldValidate: true })} className={`rounded-md px-3 py-1.5 ${clienteMode === 'registrado' ? 'bg-surface font-medium text-ink shadow-sm' : 'text-text-muted'}`}>Cliente registrado</button>
                <button type="button" onClick={() => setValue('cliente_mode', 'nuevo', { shouldValidate: true })} className={`rounded-md px-3 py-1.5 ${clienteMode === 'nuevo' ? 'bg-surface font-medium text-ink shadow-sm' : 'text-text-muted'}`}>Nuevo cliente</button>
              </div>
            </div>
            <div className="p-4">
              {clienteMode === 'registrado' ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field id="cliente_razon_social" label="Razón social" required error={errors.cliente_razon_social?.message} {...register('cliente_razon_social')} />
                  <Field id="cliente_contacto" label="Contacto" {...register('cliente_contacto')} />
                  <Field id="cliente_telefono" label="Teléfono" {...register('cliente_telefono')} />
                  <Field id="cliente_email" type="email" label="Correo (opcional)" error={errors.cliente_email?.message} {...register('cliente_email')} />
                  <Field id="cliente_direccion" label="Dirección" className="sm:col-span-2" {...register('cliente_direccion')} />
                </div>
              )}
            </div>
          </Card>
          ...
          <Card className="p-0 overflow-hidden">
            <div className="border-b border-border px-4 py-3"><CardTitle className="text-label font-medium">Trabajo solicitado</CardTitle></div>
            <div className="grid gap-4 p-4 sm:grid-cols-2">
              <div className="space-y-1.5 sm:col-span-2">
                <label htmlFor="descripcion_pieza" className="block text-label text-text-secondary">Descripción de la pieza o trabajo<span className="ml-1 text-error" aria-hidden="true">*</span></label>
                <textarea id="descripcion_pieza" rows="3" className={`input resize-none ${errors.descripcion_pieza ? 'border-error focus:ring-error' : ''}`} {...register('descripcion_pieza')} />
                {errors.descripcion_pieza && <p className="text-metadata text-error">{errors.descripcion_pieza.message}</p>}
              </div>
              <Field id="cantidad" type="number" min="1" label="Cantidad de piezas" required error={errors.cantidad?.message} {...register('cantidad')} />
              <Field id="fecha_esperada_entrega" type="date" label="Fecha de entrega solicitada (opcional)" {...register('fecha_esperada_entrega')} />
              <div className="space-y-1.5 sm:col-span-2">
                <label htmlFor="notas_comerciales" className="block text-label text-text-secondary">Notas del cliente (opcional)</label>
                <textarea id="notas_comerciales" rows="3" className="input resize-none" {...register('notas_comerciales')} />
              </div>
            </div>
          </Card>

          <Card className="p-0 overflow-hidden">
            <div className="border-b border-border px-4 py-3"><CardTitle className="text-label font-medium">Documentos</CardTitle></div>
            <div className="p-4">
          <div
            role="button"
            tabIndex={0}
          ...
          >
            ...
            <p className="mt-1 text-body text-text-secondary">Arrastrá archivos aquí o</p>
            <span className="mt-2 inline-flex rounded-md border border-border bg-surface px-3 py-1.5 text-label font-medium text-ink">Adjuntar documento</span>
